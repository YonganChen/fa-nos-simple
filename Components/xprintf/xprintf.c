...
#if _USE_XFUNC_OUT
#include <stdarg.h>
void (*xfunc_out)(uint32_t);	/* Pointer to the output stream */
...
void xputc (char c)
{
	...
	if (xfunc_out) xfunc_out((uint32_t)c);
}
...
void xfputs (					/* Put a string to the specified device */
	void(*func)(uint32_t),	/* Pointer to the output function */
	const char*	str				/* Pointer to the string */
)
{
	void (*pf)(uint32_t);
...
#if _USE_XFUNC_IN
uint32_t (*xfunc_in)(void);	/* Pointer to the input stream */
...
int xfgets (	/* 0:End of stream, 1:A line arrived */
	uint32_t (*func)(void),	/* Pointer to the input stream function */
	char* buff,	/* Pointer to the buffer */
	int len		/* Buffer length */
)
{
